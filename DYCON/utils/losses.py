...
class FeCLoss(nn.Module):
    def __init__(self, device, temperature = 0.6):
    ...
    def forward(self, feat, mask):
        ...
        feat_logits =  torch.div(torch.matmul(feat, feat.transpose(1,2)), self.temperature)
        identity = torch.eye(feat_logits.shape[-1]).to(self.device)
