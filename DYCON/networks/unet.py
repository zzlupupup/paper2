...
class UNetDecoder(nn.Module):
    ...
    def forward(self, skips):
        ...
        skips: 
        [
        torch.Size([4, 30, 64, 96, 96]),
        torch.Size([4, 63, 32, 48, 48]),
        torch.Size([4, 126, 16, 24, 24]),
        torch.Size([4, 255, 8, 12, 12]),
        torch.Size([4, 318, 4, 6, 6]),
        torch.Size([4, 318, 2, 3, 3])
        ]
        """
        skips = skips[::-1]     # reverse so that bottleneck is the first
        x = skips.pop(0)        # bottleneck # x skip: torch.Size([4, 318, 2, 3, 3])
...
class MultiEncoderUNet(nn.Module):
    ...
    def forward(self, x:torch.Tensor): # x: torch.Size([4, 3, 128, 128, 128])
