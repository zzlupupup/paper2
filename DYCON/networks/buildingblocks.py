...
class ResNetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Introduce the MCC concept here.
...
def create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding,
    ...
    encoders = []
    for i, out_feature_num in enumerate(f_maps): # [64, 128, 256, 512] 0=64, 1=128, 2=256, 3=512
        if i == 0:
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order,
    ...
    decoders = []
    reversed_f_maps = list(reversed(f_maps)) # (512, 256, 128, 64)
...
if __name__=="__main__":
    x = torch.rand((4, 3, 16, 64, 64))
    # sc = SingleConv(in_channels=3, out_channels=16, is3d=True)
    # print(f'sc: {sc}')
    # dc = DoubleConv(in_channels=3, out_channels=16, encoder=False, is3d=True)
    # print(f'dc: {dc}')
    # rb = ResNetBlock(in_channels=3, out_channels=32, is3d=True)
    # print(f'rb: {rb}')
    # se = ResNetBlockSE(in_channels=3, out_channels=32)
    # print(f'se: {se}')
    enc = Encoder(in_channels=3, out_channels=64, basic_module=ResNetBlock, is3d=True)
    out = enc(x)
