...
class AbstractUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        if isinstance(f_maps, int):
            f_maps = number_of_features_per_level(f_maps, num_levels=num_levels) # Returns a list of f_maps*2**(0-num_levels-1) ([64, 128, 256, 512])
...
if __name__=="__main__":
    x = torch.rand((4, 3, 64, 128, 128))
    # unet3d = UNet3D(in_channels=3, out_channels=2)
    # unet3d = ResidualUNet3D(in_channels=3, out_channels=2)
    unet3d = ResidualUNetSE3D(in_channels=1, out_channels=2)
    out = unet3d(x)
    print(f'out: {out.size()}')
    print(f'unet3d: {unet3d}')
