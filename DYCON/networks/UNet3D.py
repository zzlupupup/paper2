...
class UNET_3D(nn.Module):
    ...
    def forward(self, inputs):
        ...
        center = self.dropout1(center) # is the deeper feature for the fusion with the final
...
if __name__=="__main__":
    import torch
    x = torch.randn((4, 1, 64, 112, 112)).cuda()
    model = UNET_3D(in_channels=1, n_classes=1).cuda()
    out = model(x)
