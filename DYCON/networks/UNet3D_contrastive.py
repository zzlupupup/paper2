...
from .utils import UnetConv3, UnetUp3_CT
...
class UNet3D(nn.Module):
    def __init__(self, in_channels=3, feature_scale=4, n_classes=2, scale_factor=4, use_aspp=True, is_deconv=True, is_batchnorm=True):
        ...
        # ASSP module
        if self.use_aspp:
            self.aspp = build_aspp3d(inplanes=filters[4], outplanes=filters[4], output_stride=16, BatchNorm=nn.BatchNorm3d)

        # Projection head
        self.projection = nn.Sequential(
            nn.Conv3d(filters[4], 512, kernel_size=1),
            nn.BatchNorm3d(512),
    ...
    def forward(self, x):
        ...
        # projection head: scale_factor 2, 4, 8
        center_ = F.interpolate(center, scale_factor=self.scale_factor, mode='trilinear', align_corners=True)
        features = self.projection(center_)

        final = self.final(up1) 
        output_sdf = self.tanh(final)
        out_seg = self.out_conv2(up1)

        return output_sdf, out_seg, features
...
if __name__=="__main__":
    import torch
    x = torch.randn((4, 1, 112, 112, 96)).cuda()
    x2 = torch.rand_like(x).cuda()
    model = UNet3D(in_channels=1, feature_scale=4, n_classes=2, scale_factor=2, use_aspp=False).cuda()
    regresion, out, feat = model(x)
    ...
    """
    center: torch.Size([4, 256, 8, 8, 5])
    out: torch.Size([4, 2, 128, 128, 80])
    regresion: torch.Size([4, 2, 128, 128, 80])
    features: torch.Size([4, 256, 16, 16, 10])
    Total Params: 6.15M
    """