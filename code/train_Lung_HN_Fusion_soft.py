...
parser.add_argument('--exp', type=str,  default='HN_Fusion_soft', help='model_name')
...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #all sup loss
            sup_loss = 0.5*(sup_l + sup_r + fusion_sup)

            #unsup loss MSE
            fusion_unsup_l = losses.softmax_mse_loss(outputs_l[labeled_bs:], all_fusion[labeled_bs:])
            fusion_unsup_r = losses.softmax_mse_loss(outputs_r[labeled_bs:], all_fusion[labeled_bs:])
            fusion_loss = fusion_unsup_l + fusion_unsup_r
            ...
            if iter_num % 100 == 0:
                ...
                l_img_show = volume_batch[0, 0, :, :, 32].detach().cpu().numpy()
                l_label_show = label_batch[0, :, :, 32].detach().cpu().numpy()
                l_r_show = torch.argmax(torch.softmax(outputs_r[0, :, :, :, 32], dim=0), dim=0).detach().cpu().numpy()
                l_fusion_show = torch.argmax(torch.softmax(all_fusion[0, :, :, :, 32], dim=0), dim=0).detach().cpu().numpy()

                u_img_show = volume_batch[labeled_bs, 0, :, :, 32].detach().cpu().numpy()
                u_fusion_show = torch.argmax(torch.softmax(all_fusion[labeled_bs, :, :, :, 32], dim=0), dim=0).detach().cpu().numpy()
